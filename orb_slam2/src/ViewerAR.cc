...
void ViewerAR::Run()
{
    ...
    pangolin::Var<bool> menu_save("menu.Save",false,false);
    pangolin::Var<bool> menu_load("menu.Load",false,false);
    menu_load = false;
...
//        if(menu_drawim)
        ...
        if(status==2)
        {
            if (!is_cubes_loaded && menu_load)
            {
                is_cubes_loaded = true;
                menu_load = false;
	        string line;
		float nx, ny, nz, ox, oy, oz;
		char delimiter;
		ifstream myfile("/home/michael/catkin_ws/src/orb_slam_2_ros/PlaneCoordinates.txt");
		if (myfile.is_open())
		{
			while ( getline (myfile,line) )
			{
			  istringstream iss(line);
			  iss >> nx >> delimiter >> ny >> delimiter >> nz >> delimiter
			      >> ox >> delimiter >> oy >> delimiter >> oz;
			  cout << nx << " " << ny << " " << nz << " " << ox << " " << oy << " " << oz << endl;

	                  Plane* pPlane = new Plane(nx, ny, nz, ox, oy, oz);
			  if(pPlane)
			  {
			      cout << "Cube loaded" << endl;
...
//                        }
		    ...
		    if(!vpPlane.empty())
                    {
                            ofstream myfile;
			    myfile.open ("/home/michael/catkin_ws/src/orb_slam_2_ros/PlaneCoordinates.txt");
			    for(size_t i=0; i<vpPlane.size(); i++)
			    {
				Plane* pPlane = vpPlane[i];
				if(pPlane)
				{
			             myfile << pPlane -> n.at<float>(0,0) << "," 
				            << pPlane -> n.at<float>(1,0) << ","
                                            << pPlane -> n.at<float>(2,0) << "," 
					    << pPlane -> o.at<float>(0,0) << ","
                                            << pPlane -> o.at<float>(1,0) << ","
                                            << pPlane -> o.at<float>(2,0) << endl; 	
				}
			    }
			    myfile.close(); 
                    }
...
void ViewerAR::SaveMap()
{
    bool res = mpSystem->SaveMap(mpSystem->map_file);
    cout << "Save File Result: " << res << endl;
}

void ViewerAR::LoadMap()
{
    bool res = mpSystem->LoadMap(mpSystem->map_file);
    cout << "Load File Result: " << res << endl;
}
